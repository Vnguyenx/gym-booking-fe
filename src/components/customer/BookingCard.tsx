...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onCancel }) => {
    ...
    const handleCancel = async () => {
        const confirmed = window.confirm('Bạn có chắc muốn huỷ đăng ký này không?');
        if (!confirmed) return;
            ...
            <div className="booking-card__top">
                <div>
                    <p className="booking-card__service">
                        {MEMBERSHIP_CONFIG[booking.membershipId] ?? booking.membershipId}
                    </p>
                    <p className="booking-card__meta">
                        PT: {getPTName(booking.ptId) || 'Không có'} · {PT_SERVICE_CONFIG[booking.ptServiceId] ?? booking.ptServiceId}
                    </p>
                    <p className="booking-card__meta">
                        Ngày đăng ký: {formatDate(booking.createdAt)}
                    </p>
                </div>
